...
<div class="page-header">
  <div>
    ...
    <a href="<?= BASE_PATH ?>/projects/view.php?id=<?= h($estimate['proj_id']) ?>" class="muted"><?= h($estimate['customer_name']) ?> / <?= h($estimate['project_name']) ?></a>
  </div>
  <div class="actions">
    <a href="<?= BASE_PATH ?>/estimates/edit.php?id=<?= h($id) ?>" class="btn btn-outline">編集</a>
    <form method="post" action="<?= BASE_PATH ?>/estimates/delete.php" onsubmit="return confirm('見積書を削除しますか?この操作は取り消せません。');" style="display:inline;">
      <?= csrf_field() ?>
