...
$values = $estimate;
$items = !empty($existingItems) ? $existingItems : [['name' => '', 'quantity' => '1', 'unit' => '', 'unit_price' => '0', 'notes' => '']];
$errors = [];
    ...
    if (empty($errors)) {
        ...
        $stmt = db()->prepare(
            'UPDATE estimates SET
               project_id=?, title=?, addressee=?, issuer_name=?, issuer_address=?, issuer_contact=?,
               issue_date=?, valid_until=?, tax_rate=?, tax_type=?, subtotal=?, tax_amount=?, total_amount=?, status=?, notes=?
             WHERE id=?'
        );
        $stmt->execute([
            $data['project_id'], $data['title'], $data['addressee'], $data['issuer_name'], $data['issuer_address'], $data['issuer_contact'],
...
<form method="post" class="card">
  ...
  <div class="field">
    ...
    <input type="text" id="addressee" name="addressee" value="<?= v($values, 'addressee') ?>">
  ...
  <div class="row-4">
    <div class="field">
      ...
      <input type="date" id="issue_date" name="issue_date" value="<?= h(format_date($values['issue_date'] ?? '')) ?>">
      <?= err($errors, 'issue_date') ?>
    </div>
    <div class="field">
      <label for="valid_until">有効期限</label>
      <input type="date" id="valid_until" name="valid_until" value="<?= h(($values['valid_until'] ?? '') ? format_date($values['valid_until']) : '') ?>">
    </div>
    <div class="field">
